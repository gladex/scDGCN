...
class load_data(Dataset):
    def __init__(self, dataset):
        #self.x = pd.read_csv('data/MTAB/mtab.txt', sep='\t', index_col=0)
        #self.x = np.array(self.x.values)
        self.x=sc.read_csv('data/PBMC/pbmc.csv')
        #print(self.x)
        #self.y = np.loadtxt('data/MTAB/{}_label.txt'.format(dataset),dtype=int)
        self.y= pd.read_csv('data/PBMC/clusters.csv', index_col=0)
        self.y= np.array(self.y.values)
        #adata.obs['Group'] = self.y
...
def load_graph(n, k):
    if k:
        ...
        path = 'graph/PBMC_ncos_graph.txt'

    else:
        #path = 'graph/{}_graph.txt'.format(dataset)
        path ='graph/PBMC_ncos_graph.txt'

    #data = np.loadtxt('data/MTAB/{}.txt'.format(dataset))
    #data = sc.read_csv('data/PBMC/pbmc.csv')
...
def normalization_features(features_):
...
def Decoding_loss2(x_bar, data,adj):
    a = 0.7
    b = 0.7
    ...
    x_m = weighted_features_all- x_bar
    #x_m.cpu()
    loss2 =F.mse_loss(x_bar,weighted_features_all)
    #loss2 = torch.norm(x_m) ** 2
    loss = a*loss1 + b*loss2
    return loss